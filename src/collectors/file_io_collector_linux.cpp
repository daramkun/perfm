...
public:
    ...
    {
        pid_ = pid;
        last_read_ = read_counter("read_bytes:");
        last_write_ = read_counter("write_bytes:");
    }

    std::vector<metric_value> sample() override
    {
        const auto current_read = read_counter("read_bytes:");
        const auto current_write = read_counter("write_bytes:");
        const auto read_delta = current_read - last_read_;
        const auto write_delta = current_write - last_write_;
...
private:
    unsigned long long read_counter(const std::string& key) const
    {
        std::ifstream input("/proc/" + std::to_string(pid_) + "/io");
        std::string label;
